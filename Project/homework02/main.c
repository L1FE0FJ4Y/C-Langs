...
{
  if (input_one && input_two && output && num_ints > 0)
  {
    input_one = malloc(num_ints * MAX_NUM_LENGTH * sizeof(int));
    input_two = malloc(num_ints * MAX_NUM_LENGTH * sizeof(int));
    output = malloc(num_ints * MAX_NUM_LENGTH * sizeof(int));

    unsigned int resetValue = 0;
    for (int i = 0; i < num_ints; ++i)
    {
      input_one[i] = resetValue;
      input_two[i] = resetValue;
      output[i] = resetValue;
    }
  }
  else
  {
    printf("Size of num_ints is invalid.\n");
    exit(EXIT_FAILURE);
  }
}
              ...
              unsigned long int* output, int num_ints)
{
  ...
  for (int i = 0; i < num_ints; ++i)
  {
    fscanf(fp1[i], "%d", input_one[i]);
    fscanf(fp2[i], "%d", input_two[i]);
  }
  fclose(fp1);
  fclose(fp2);
                 ...
                 unsigned long int* output, int num_ints)
{
  ...
  fwrite(&output, sizeof(output), 1, argv[3]);
  fclose(output);
}
